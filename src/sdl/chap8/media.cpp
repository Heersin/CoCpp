...
int main()
{
    ...
    while(!quit)
    {
        ...
        SDL_SetRenderDrawColor(base_render, 0xFF, 0x00, 0x00, 0xFF);
        SDL_RenderFillRect(base_render, &fill_rect);

        // Render green outlined quad
        SDL_Rect outline_rect = {SCREEN_WIDTH / 6, SCREEN_HEIGHT / 6, SCREEN_WIDTH * 2 / 3, SCREEN_HEIGHT * 2 / 3};
        SDL_SetRenderDrawColor(base_render, 0x00, 0xFF, 0x00, 0xFF);
        SDL_RenderDrawRect(base_render, &outline_rect);

        // Draw Blue horizontal
        SDL_SetRenderDrawColor(base_render, 0x0, 0x0, 0xFF, 0xFF);
        SDL_RenderDrawLine(base_render, 0, SCREEN_HEIGHT / 2, SCREEN_WIDTH, SCREEN_HEIGHT / 2);

        // Draw a series of dots yellow ()
        SDL_SetRenderDrawColor(base_render, 0xFF, 0xFF, 0x00, 0xFF);
        for ( int i = 0; i < SCREEN_HEIGHT; i += 4)
            SDL_RenderDrawPoint(base_render, SCREEN_WIDTH / 2, i);
